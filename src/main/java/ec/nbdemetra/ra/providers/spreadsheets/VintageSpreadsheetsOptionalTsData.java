...
import ec.nbdemetra.ra.providers.txt.*;
 ...
 * @author bennouha
 */
public abstract class VintageSpreadsheetsOptionalTsData {

    public static VintageSpreadsheetsOptionalTsData present(int nbrRows, int nbrUselessRows, TsData data) {
        Preconditions.checkNotNull(data);
        Preconditions.checkArgument(nbrRows >= nbrUselessRows && nbrUselessRows >= 0);
        return new Present(nbrRows, nbrUselessRows, data);
    }

    public static VintageSpreadsheetsOptionalTsData absent(int nbrRows, int nbrUselessRows, String cause) {
    ...
    private VintageSpreadsheetsOptionalTsData(int nbrRows, int nbrUselessRows) {
    ...
    private static class Present extends VintageSpreadsheetsOptionalTsData {

        private final TsData data;
        private VintageTsDataCollector dc = new VintageTsDataCollector();
    ...
    private static class Absent extends VintageSpreadsheetsOptionalTsData {
    ...
    public static class Builder implements IBuilder<VintageSpreadsheetsOptionalTsData> {
        ...
        @Override
        public VintageSpreadsheetsOptionalTsData build() {
            if (dc.getCount() == 0) {
        ...
        private VintageSpreadsheetsOptionalTsData onSuccess(TsData tsData) {
            return new Present(dc.getCount(), nbrUselessRows, tsData, dc);
        }

        private VintageSpreadsheetsOptionalTsData onFailure(String cause) {
