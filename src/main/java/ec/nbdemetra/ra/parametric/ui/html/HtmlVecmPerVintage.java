 ...
 * @author aresda
 */
public class HtmlVecmPerVintage extends RevisionHtmlElement {

    private final ParametricSpecification specification;
    private final ComponentMatrix cpMatrix;
    private final MethodName method;
    private final String vintage;
    private final int vecmLag;

    public HtmlVecmPerVintage(ComponentMatrix cpMatrix, ParametricSpecification specification, MethodName method, String vintage) {
    ...
    public void write(HtmlStream stream) throws IOException {
        stream.write(HtmlTag.HEADER1, h1, method.toString() + " for vintage " + this.vintage).newLine();
        boolean noResult = true;
        for (int i = 0; i < cpMatrix.getRowsLabels().length; i++) {
            String[] crossVintage = ((String) cpMatrix.getRowsLabels()[i]).split("\\|");
            if (!onlyNCValues(cpMatrix.getRowsLabels()[i]) && crossVintage[0].equals(this.vintage)) {
        ...
        if (noResult) {
            stream.write(HtmlTag.HEADER2, cssError , ONLY_NC_VALUES).newLine();
        }
    }
